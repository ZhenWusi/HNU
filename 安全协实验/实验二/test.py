...
print("E=",E)
# 合并消息 M
M = (pow(M1, (E*pow(e1,-1,phi))%n, n) * pow(M2, (E*pow(e2,-1,phi))%n, n)) % n
# 对消息 M 进行签名
S = pow(M1,pow(e1,-1,phi),n)*pow(M2,pow(e2,-1,phi),n)
...
print("X =", X)
# 划分出每一条消息的签名
signature_M2 = (pow(S, X, n) * pow(pow(M1, X*pow(e1,-1,phi), n) * pow(M2,(X-1)*pow(e2,-1,phi), n), -1,n)) %n
signature_M1 = (S*pow(pow(M2,(pow(e2,-1,phi)),n),-1,n)) % n
print("消息 M 的合并结果:", M)
print("批签名 S:", S)
print("从批签名中划分出单个消息m1的签名:", signature_M1)
print("M1单个签名",pow(M1,pow(e1,-1,phi),n))
print("从批签名中划分出单个消息m2的签名:", signature_M2)
print("M2单个签名",pow(M2,pow(e2,-1,phi),n))
